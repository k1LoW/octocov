...
func (b *BQ) FS(table string) (fs.FS, error) {
	...
	fsys := fstest.MapFS{}
	t := fmt.Sprintf("`%s.%s`", b.dataset, table)
	q := fmt.Sprintf(`SELECT r.owner, r.repo, r.timestamp, r.raw FROM %s AS r
INNER JOIN (
...
ORDER BY r.owner, r.repo`, t, t)
	it, err := b.client.Query(q).Read(ctx)
	if err != nil {
		return nil, err
	}
	for {
