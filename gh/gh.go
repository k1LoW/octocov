...
// ListWorkflowJobs fetches the jobs of the current workflow run (GITHUB_RUN_ID),
// retrying with backoff on transient API errors. Callers that need to wait for a
// job to appear or a step to complete should call this again themselves inside
// their own backoff loop, resolving job IDs and looking up steps against the same
// fetched snapshot so the jobs are only fetched once per attempt.
func (g *Gh) ListWorkflowJobs(ctx context.Context, owner, repo string) (*Jobs, error) {
...
// Jobs is a snapshot of a workflow run's jobs, fetched once and shared by the
// job-ID resolution and step lookups performed against it.
type Jobs struct {
	jobs *github.Jobs
}

// ResolveIDs returns the IDs of the job(s) to search for test execution time
// steps. If jobPatterns is empty, it falls back to the single job octocov itself
// is running in (matching on GITHUB_JOB, or on a currently running step whose name
// looks like octocov/coverage). Otherwise, it returns the IDs of every job whose
// name matches any of the glob patterns, e.g. "test (*)" matches the jobs
// generated by a GitHub Actions matrix such as "test (1)", "test (2)".
func (js *Jobs) ResolveIDs(jobPatterns []string) ([]int64, error) {
	if len(jobPatterns) == 0 {
		if len(js.jobs.Jobs) == 1 {
			return []int64{js.jobs.Jobs[0].GetID()}, nil
		}
		for _, j := range js.jobs.Jobs {
			if j.GetName() == os.Getenv("GITHUB_JOB") {
				return []int64{j.GetID()}, nil
			}
	...
	for _, j := range js.jobs.Jobs {
		for _, pattern := range jobPatterns {
			ok, err := doublestar.Match(pattern, j.GetName())
			if err != nil {
				return nil, err
			}
			if ok {
...
// FindStepByTime finds, among jobIDs, the completed step whose execution window
// contains t. ok is false if no such step exists in the snapshot yet.
func (js *Jobs) FindStepByTime(jobIDs []int64, t time.Time) (step Step, ok bool) {
	for _, j := range js.jobs.Jobs {
		...
		for i, s := range j.Steps {
			log.Printf("job %d step [%d/%d]: %s %v-%v", j.GetID(), i+1, l, s.GetName(), s.StartedAt, s.CompletedAt) //nolint:gosec // values from GitHub API response, not user input
			...
			if startTime.Unix() < t.Unix() && t.Unix() <= completeTime.Unix() {
				...
				return Step{Name: s.GetName(), StartedAt: startTime, CompletedAt: completeTime}, true
			}
		}
	}
	return Step{}, false
...
// FindStepsByName finds every step named name among jobIDs. ok is false if no
// step with that name exists yet in the snapshot, or any matching step has not
// completed.
func (js *Jobs) FindStepsByName(jobIDs []int64, name string) (steps []Step, ok bool) {
	matched := 0
	for _, j := range js.jobs.Jobs {
		if !slices.Contains(jobIDs, j.GetID()) {
			continue
