...
// ResolveJobIDs resolves which job(s) of the current workflow run should be
// searched for test execution time steps. If jobPatterns is empty, it falls back
// to the single job octocov itself is running in (see DetectCurrentJobID).
// Otherwise, it returns the IDs of every job in the run whose name matches any of
// the glob patterns, e.g. "test (*)" matches the jobs generated by a GitHub Actions
// matrix such as "test (1)", "test (2)".
func (g *Gh) ResolveJobIDs(ctx context.Context, owner, repo string, jobPatterns []string) ([]int64, error) {
	if len(jobPatterns) == 0 {
		id, err := g.DetectCurrentJobID(ctx, owner, repo)
		if err != nil {
			return nil, err
		}
		return []int64{id}, nil
	...
	return nil, fmt.Errorf("could not find any job matching %v", jobPatterns)
}

// FetchStepByTimeAcrossJobs finds the step, searching every job in jobIDs, whose
// execution window contains t. Unlike FetchStepByTime (which looks at a single
// job), this also finds the step when it ran in a different job than the one
// octocov itself is running in, e.g. when test jobs are split by a GitHub Actions
// matrix and the coverage report is measured in a separate aggregation job.
func (g *Gh) FetchStepByTimeAcrossJobs(ctx context.Context, owner, repo string, t time.Time, jobIDs []int64) (Step, error) {
	if os.Getenv("GITHUB_RUN_ID") == "" {
		return Step{}, fmt.Errorf("env %s is not set", "GITHUB_RUN_ID")
	}
	runID, err := strconv.ParseInt(os.Getenv("GITHUB_RUN_ID"), 10, 64)
	if err != nil {
		return Step{}, err
	...
	return Step{}, fmt.Errorf("the step that was executed at the relevant time (%v) does not exist in run (%d)", t, runID)
}

func (g *Gh) FetchStepsByName(ctx context.Context, owner, repo string, name string, jobIDs []int64) ([]Step, error) {
	if os.Getenv("GITHUB_RUN_ID") == "" {
		return nil, fmt.Errorf("env %s is not set", "GITHUB_RUN_ID")
	}
	runID, err := strconv.ParseInt(os.Getenv("GITHUB_RUN_ID"), 10, 64)
