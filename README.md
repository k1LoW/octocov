...
- The artifact is named after the report, `octocov-report[-<key>]@refs_pull_<number>.html`, and a re-run of the workflow names its page `octocov-report[-<key>]@refs_pull_<number>@attempt_<attempt>.html`, since a run holds one artifact of a name. Once the comment, the job summary and the body are all written, the pages earlier attempts and earlier runs of the same pull request uploaded are deleted, which needs `actions: write`. Without it they are left until their retention runs out.
