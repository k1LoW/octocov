...
The same goes for the files the diff of the merge commit leaves out once it reaches its
[limit of 300 files](https://docs.github.com/en/rest/commits/commits): the list of changed files
still comes from the pull request, so such files stay in `patch`, numbered as the lines of the pull
request. Whenever changed lines are left numbered that way, octocov says so and why in the job log.
...
- The overall coverage links to the page, and the coverage of each file links to the card of that file on it. A file the page drew no card for, since it stops drawing cards once a page holds enough of them, links to the page itself. The compared column gets no link, since the page shows both reports side by side.
- The hunks are taken the way the changed lines of `patch` are, from the diff of the merge commit the coverage was measured on against its first parent, so the coverage beside each line is that line's. The base column is drawn when the compared report was taken at the commit the old side of that diff is numbered as. Where the changed lines are left numbered as the pull request's, as on `pull_request_target`, the page draws no coverage of the head beside the hunks and says why.
...
- The artifact is named after the report, `octocov-report[-<key>]@refs_pull_<number>.html`, and a re-run of the workflow names its page `octocov-report[-<key>]@refs_pull_<number>@attempt_<attempt>.html`, since a run holds one artifact of a name. Once the comment, the job summary and the body are all written, the pages earlier attempts of this run and the first attempts of earlier runs of the same pull request uploaded are deleted, which needs `actions: write`. Without it they are left until their retention runs out, and so is a page an earlier run uploaded on a re-run of its own, since its name carries an attempt the next run cannot know.
...
The report of every ref is stored under the artifact name as it is configured. Beside it, each run stores an artifact of metadata that points, by ID, at the artifact holding the report of the ref the run is on. It is named `octocov-metadata-`, the artifact name, `@` and the ref, with the characters an artifact name may not hold replaced by `_` ( e.g. `octocov-metadata-octocov-report@refs_heads_main`, `octocov-metadata-octocov-report@refs_pull_123` ).
